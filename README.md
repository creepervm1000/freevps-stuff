# freevps-stuff
basically a list of free vps providers (these are mostly only trials like you need a cc for most tho)
# 1: the always free ones that need CCs
...
# 4 the no cc trials
now i have 2 here. 1: cloudsigma, they offer a 7day trial. any os, SLOW VNC you should use rdp or ssh after installing. the net is limited to 5gb transfer only (this is not speed) on the trial. needs a valid phone number which you can get from quackr or just enter a real one. also you need a business email for this (all ports open + dynamic ip)<br>browseruse bux: well they call it agency but i still refer to it as bux. they give u aws env you can use after linking a tg bot and claude code or codex. now you actually dont need to do this since u can use the network tab to see the shell url but i wont go into detail here. u can setup ssh on it. better specs than cloudsigma. same 7 day length. only port 22 open (they use aws and its security policys + dynamic ip)
